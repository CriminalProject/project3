...
import csv
from myOpt.models import OptimizationData
...
def readAndWriteData(request):
    ...
    for row in csv_reader:
       ...
       # counter = counter +1
    #print(counter)
        variables = OptimizationData(date = d, store = row[1],location = row[2],code = row[3],salerProductName = row[4],mainGroup = row[5],subGroup = row[6],productVariety = row[7],salesAmount = row[8])
        variables.save();
    csv_file.close()
...
def readFromDB():
     opt = OptimizationData.objects.filter()
...
# Iterate over the data and write it out row by row.
     for obj in opt:
        ...
        worksheet.write(row, col + 1, obj.store)
